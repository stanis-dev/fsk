...
func resByID(t *testing.T, rs []checkResult, id string) checkResult {
	t.Helper()
	for _, r := range rs {
		if r.ID == id {
			return r
	...
	t.Fatalf("missing check result %q in %+v", id, rs)
	return checkResult{}
}
...
func TestRunChecks(t *testing.T) {
	...
	if !resByID(t, rs, "groundedBeforeWrite").Pass {
		t.Error("grounded should pass (search at 0 < edit at 1)")
	}
	if !resByID(t, rs, "toolsCalled:search_fiskaly_docs").Pass {
		t.Error("search called 2x should meet min 2")
	}
	if resByID(t, rs, "toolsCalled:Bash").Pass {
		t.Error("Bash never called should fail")
	}
	if !resByID(t, rs, "docsFetched:probe:records-flow").Pass {
		t.Error("probe:records-flow fetched doc id should pass")
	}
	if resByID(t, rs, "docsFetched:probe:auth-and-headers").Pass {
		t.Error("probe:auth-and-headers not fetched should fail")
	}
	if resByID(t, rs, "docsFetched:missing-doc").Pass {
		t.Error("missing-doc not fetched should fail")
	}
	if resByID(t, rs, "maxMcpErrors").Pass {
