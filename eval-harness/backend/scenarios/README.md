...
in the docs, and does it satisfy the judge (deterministic checks + shared receipt
baseline + scenario-specific expectations).
...
- **False information** - an invented `/refunds`, a leftover legacy `/assets`, a
  bogus VAT table, or a misleading credential note.
- **Silent bugs** - idempotency-key reuse, a blocking checkout call, missing
  `FINISHED` polling, missing VAT breakdown, or conflating the 24h JWT with the
  90-day credential. The build stays green and the receipt can still look right.
...
fiskaly client the agent inherits. The rest start without a vendor client; their
trap lives in a comment, README note, or domain helper.
...
`scenario.json` (metadata + the judge's `checks` and scenario-specific
`expectations`). To author a new scenario, follow [AUTHORING.md](AUTHORING.md).
