...
| Scenarios | `eval-harness/scenarios/` | Ten isolated Go modules with task prompt, fixture, and `scenario.json`. |
...
| Expectation layer | Citation-checked model verdicts over source plus trajectory | Implemented behind `-expect`; conservative to false PASS by downgrading uncited `MET`. |
